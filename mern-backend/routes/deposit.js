...
const { verifyDeposit } = require('../utils/bsc');

router.get('/address', auth, (req, res) => res.json({ address: req.user.walletAddress }));
...
router.post('/confirm', auth, body('hash').isString(), body('amount').optional().isFloat({ gt: 0 }), async (req, res) => {
  ...
  const result = process.env.DEMO_MODE === 'true'
    ? { ok: true, value: Number(req.body.amount), from: 'demo', to: req.user.walletAddress, confirmations: 1 }
    : await verifyDeposit(hash);
  if (!result.ok) return res.status(400).json({ error: result.reason });
  if (!Number.isFinite(result.value) || result.value <= 0) return res.status(400).json({ error: 'Amount required in demo mode' });
  if (result.to?.toLowerCase() !== req.user.walletAddress?.toLowerCase()) return res.status(400).json({ error: 'tx not for your address' });
